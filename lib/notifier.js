...
function notifyUpdateThreshold() {
    if (ExtensionUtils.getSettings().get_boolean('show-notifications')) {
        ...
        if (Driver.deviceInfo[type][0] === '1') {
            if (Driver.deviceInfo[type][1] === '1')
                notify(_('Battery 1\nCharge thresholds are set to %d / %d %%').format(Driver.getCurrentEndLimitValue(), Driver.getCurrentStartLimitValue()), 'update');
            else
                notify(_('Charge thresholds are set to %d / %d %%').format(Driver.getCurrentEndLimitValue(), Driver.getCurrentStartLimitValue()), 'update');
        } else  if (Driver.deviceInfo[type][1] === '1') {
            notify(_('Battery 1\nCharging Limit is set to %d%%').format(Driver.getCurrentEndLimitValue()), 'update');
        } else {
            notify(_('Charging Limit is set to %d%%').format(Driver.getCurrentEndLimitValue()), 'update');
        }
    }
}

function notifyUpdateThreshold2() {
    if (ExtensionUtils.getSettings().get_boolean('show-notifications')) {
        const type = ExtensionUtils.getSettings().get_int('device-type');
        if (Driver.deviceInfo[type][0] === '1')
            notify(_('Battery 2\nCharge thresholds are set to %d / %d %%').format(Driver.getCurrentEndLimit2Value(), Driver.getCurrentStartLimit2Value()), 'update');
        else
            notify(_('Battery 2\nCharging Limit is set to %d%%').format(Driver.getCurrentEndLimit2Value()), 'update');
    }
}
