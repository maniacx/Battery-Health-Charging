...
const ChargeLimitToggle = GObject.registerClass(
    class ChargeLimitToggle extends QuickSettings.QuickMenuToggle {
        _init() {
            ...
            this._preferencesButton.child = new St.Icon({
            ...
            // update menu UI
            this._notify = 0; // 0-Dont notify, 1-notifyUpdateInthreshold1, 2-notifyUpdateInthreshold2
        ...
        _updatePanelMenu() {
            ...
            // Read updated threshold values from sysfs
            ...
            if (Driver.deviceInfo[this.type][0] === '1') {
                ...
                if (this._notify === 1)
                    Notify.notifyUpdateThreshold(this.currentEndLimitValue, this.currentStartLimitValue);
            } else if (this._notify === 1) {
                Notify.notifyUpdateThreshold(this.currentEndLimitValue, 0);
            }
            if (Driver.deviceInfo[this.type][1] === '1') {
                ...
                if (Driver.deviceInfo[this.type][0] === '1') {
                    ...
                    if (this._notify === 2)
                        Notify.notifyUpdateThreshold2(this.currentEndLimit2Value, this.currentStartLimit2Value);
                } else if (this._notify === 2) {
                    Notify.notifyUpdateThreshold2(this.currentEndLimit2Value, 0);
                }
            }
            ...
            if (Driver.deviceInfo[this.type][1] === '1') {
                if (!this._settings.get_boolean('show-battery-panel2')) {
                    this.label = _('Battery 1');
                    this.setHeaderLabel = _('Battery 1 Charging Mode');
                } else {
                    this.label = _('Battery 2');
                    this.setHeaderLabel = _('Battery 2 Charging Mode');
                }
            } else  {
                this.setHeaderLabel = _('Battery Charging Mode');
                this.label = _('Battery');
            }

            const headerIconName = `charging-limit-${this.iconType}-${this.currentModeIcon}-symbolic`;
            this.gicon = getIcon(headerIconName);
            this.menu.setHeader(getIcon(headerIconName), this.setHeaderLabel);
        }
        ...
        _updatePanelMenuBattery1() {
            ...
            if (Driver.deviceInfo[this.type][0] === '1') {
                currentLimitValueString = _('Device will stop charging at %d%%\nDevice will start charging at %d%%')
                    .format(this.currentEndLimitValue, this.currentStartLimitValue);
            } else {
        ...
        _updatePanelMenuBattery2() {
            ...
            if (Driver.deviceInfo[this.type][0] === '1') {
                currentLimit2ValueString = _('Device will stop charging at %d%%\nDevice will start charging at %d%%')
                    .format(this.currentEndLimit2Value, this.currentStartLimit2Value);
            } else {
