...
async function runCommandCtl(device, endLimit, startLimit, retStdout) {
    const ctlPath = ExtensionUtils.getSettings().get_string('ctl-path');
    let argv = ['pkexec', ctlPath, device];
    if (endLimit !== null) {
        argv = argv.concat(endLimit);
        if (startLimit !== null)
            argv = argv.concat(startLimit);
    }
    return await execCheck(argv, false, retStdout);
}
