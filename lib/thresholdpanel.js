    ...
    class ThresholdPanel extends QuickSettings.SystemIndicator {
        constructor() {
            ...
            case 2:
                ...
                return;
            case 3:
                this._notify(_('Battery Health service not installed.\n' +
            ...
            let initialModeSettings = this._settings.get_string('charging-mode');
            Driver.setEndThresholdLimit(this._getCurrentEndValue(initialModeSettings));
            if (isChargeStartThresholdSupported)
                Driver.setStartThresholdLimit(this._getCurrentStartValue(initialModeSettings));
            ...
            this._settings.connectObject(
                'changed::charging-mode', () => {
                    ...
                    let currentStartLimitValue = 0;
                    let currentModeSettings = this._settings.get_string('charging-mode');
                    currentEndLimitValue = this._getCurrentEndValue(currentModeSettings);

                    if (isChargeStartThresholdSupported) {
                        currentStartLimitValue = this._getCurrentStartValue(currentModeSettings);
                        this._notify(_('Charge thresholds are set to %d / %d %%').format(currentEndLimitValue, currentStartLimitValue), 'update');
                    } else  {
                        this._notify(_('Charging Limit is set to %d%%').format(currentEndLimitValue), 'update');
                    }
                },
                'changed::install-service', () => {
                    if (flag) {
                        if (this._settings.get_boolean('install-service'))
                            this._notify(_('Installation Successfull. Please save your work and logout.'), 'installed');
                        else
                            this._notify(_('Uninstallation Successfull.'), 'uninstalled');
                    }
