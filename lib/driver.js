...
async function execCheck(argv, input = null, cancellable = null) {
    ...
    debugLog(`execCheck commad = ${argv}`);
...
async function runInstallerScript(action, asRoot) {
...
function installedCtlPath() {
    for (const name of [
        ...
        if (standard !== null) {
            debugLog(`installedCtlPath returning standard: ${standard}`);
            return standard;
        }
        for (const bindir of ['/usr/local/bin/', '/usr/bin/']) {
            const path = bindir + name;
            debugLog(`Looking for: ${path}`);
            if (Gio.File.new_for_path(path).query_exists(null)) {
                debugLog(`installedCtlPath returning path: ${path}`);
                return path;
            }
        }
    }
...
async function runInstaller() {
    const status = await runInstallerScript('install', true);
    if (status == 0)
        ExtensionUtils.getSettings().set_int('install-service', 0);
}

async function runUpdater() {
    const status = await runInstallerScript('update', true);
    if (status == 0)
        ExtensionUtils.getSettings().set_int('install-service', 0);
}

async function runUninstaller() {
    const status = await runInstallerScript('uninstall', true);
    if (status == 0)
        ExtensionUtils.getSettings().set_int('install-service', 1);
}

async function checkInstallation() {
    const status = await runInstallerScript('check', false);
    if (status == 0) {   // Polkit Installed
        ExtensionUtils.getSettings().set_int('install-service', 0);
        return 0;
    } else if (status == 3) {  // Polkit Needs Update
        ExtensionUtils.getSettings().set_int('install-service', 2);
        return 1;
    } else if (status == 5) {  // Polkit is not installed
        ExtensionUtils.getSettings().set_int('install-service', 1);
        return 2;
    } else {
        debugLog(`Installer check mode Error${status}`);
        return 3;
    }
}
...
async function checkInCompatibility() {
    ...
    if (isDeviceAuthRequired()) {
        ...
        if (installStatus === 1)
            ...
            return 6;   // Installer Error
        ExtensionUtils.getSettings().set_string('ctl-path', installedCtlPath());
        ExtensionUtils.getSettings().set_boolean('root-mode', true);
    }
    // restore mode
    const currentType = ExtensionUtils.getSettings().get_int('device-type');
