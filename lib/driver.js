...
const {fileExists, readFile, execCheck} = Helper;
...
export class IntializeDriver {
    ...
    _notifyDependencies() {
        const vendorList = {
            'Apple Inc': 'apple-m-series', 'Apple': 'apple-intel-series', 'Dell Inc.': 'dell', 'Acer': 'acer', 'MSI': 'msi',
            'Framework': 'framework', 'GIGABYTE': 'gigabyte', 'SLIMBOOK': 'slimbook', 'TUXEDO': 'tuxedo', 'Razer': 'razer',
        };
        const vendor = readFile('/sys/devices/virtual/dmi/id/sys_vendor');
        let pathSuffix = vendor && vendorList[Object.keys(vendorList).find(key => vendor.includes(key))];
        if (!pathSuffix && readFile('/sys/devices/virtual/dmi/id/board_name')?.includes('LAPQC71'))
